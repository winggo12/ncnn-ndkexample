...
class AngleBothShoulders : CriteriaBase() {
    ...
    override fun set(maxAngle: Double, step: Double, direction: String) {
        ...
        if (this.max_angle > 160){
            this.good_performance = " shoulder is straight enough"
            this.bad_performance = " shoulder is not straight enough"
        }else if (this.max_angle < 60){
            this.good_performance = " shoulder is curve enough"
            this.bad_performance = " shoulder is not curve enough"
        }else{
            this.good_performance = " shoulder is close to " + this.max_angle.toString() + " degree"
            this.bad_performance = " shoulder is not close to " + this.max_angle.toString() + " degree"
        }
    }

    private fun updateScore(){
        this.leftScore = FeedbackUtilities.left_shoulder(this.keypoint!!, this.max_angle, this.step, false)
        this.rightScore = FeedbackUtilities.right_shoulder(this.keypoint!!, this.max_angle, this.step, false)
        this.score = 0.5*this.rightScore + 0.5* this.leftScore
    }

    private fun updateColorBit(){
        this.leftColorBit = ColorUtilities.left_shoulder(this.leftScore)
        this.rightColorBit = ColorUtilities.right_shoulder(this.rightScore)
