...
class AngleBothLegs : CriteriaBase() {
    ...
    override fun set(maxAngle: Double, step: Double, direction: String) {
        ...
        if (this.max_angle > 160){
            this.good_performance = " leg is straight enough"
            this.bad_performance = " leg is not straight enough"
        }else if (this.max_angle < 60){
            this.good_performance = " leg is curve enough"
            this.bad_performance = " leg is not curve enough"
        }else{
            this.good_performance = " leg is close to " + this.max_angle.toString() + " degree"
            this.bad_performance = " leg is not close to " + this.max_angle.toString() + " degree"
        }
    }

    private fun updateScore(){
        this.leftScore = FeedbackUtilities.left_leg(this.keypoint!!, this.max_angle, this.step, false)
        this.rightScore = FeedbackUtilities.right_leg(this.keypoint!!, this.max_angle, this.step, false)
        this.score = 0.5*this.rightScore + 0.5* this.leftScore
    }

    private fun updateColorBit(){
        this.leftColorBit = ColorUtilities.left_leg(this.leftScore)
        this.rightColorBit = ColorUtilities.right_leg(this.rightScore)
