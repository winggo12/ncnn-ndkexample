...
class AngleBothWaists : CriteriaBase() {
    ...
    override fun set(maxAngle: Double, step: Double, direction: String) {
        ...
        if (this.max_angle > 160){
            this.good_performance = " waist is straight enough"
            this.bad_performance = " waist is not straight enough"
        }else if (this.max_angle < 60){
            this.good_performance = " waist is curve enough"
            this.bad_performance = " waist is not curve enough"
        }else{
            this.good_performance = " waist is close to " + this.max_angle.toString() + " degree"
            this.bad_performance = " waist is not close to " + this.max_angle.toString() + " degree"
        }
    }

    private fun updateScore(){
        this.leftScore = FeedbackUtilities.left_waist(this.keypoint!!, this.max_angle, this.step, false)
        this.rightScore = FeedbackUtilities.right_waist(this.keypoint!!, this.max_angle, this.step, false)
        this.score = 0.5*this.rightScore + 0.5* this.leftScore
    }

    private fun updateColorBit(){
        this.leftColorBit = ColorUtilities.left_waist(this.leftScore)
        this.rightColorBit = ColorUtilities.right_waist(this.rightScore)
