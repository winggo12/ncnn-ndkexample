...
class AngleSingleWaist: CriteriaBase() {
    ...
    override fun set(maxAngle: Double, step: Double, direction: String) {
        ...
        if (this.max_angle > 160){
            this.good_performance = this.direction + " waist is straight enough"
            this.bad_performance = this.direction + " waist is not straight enough"
        }else if (this.max_angle < 60){
            this.good_performance = this.direction + " waist is curve enough"
            this.bad_performance = this.direction + " waist is not curve enough"
        }else{
            this.good_performance = this.direction + " waist is close to " + this.max_angle.toString() + " degree"
            this.bad_performance = this.direction + " waist is not close to " + this.max_angle.toString() + " degree"
        }
    }

    private fun updateScore(){
        if (this.direction == LEFT){
            this.score = FeedbackUtilities.left_waist(this.keypoint!!, this.max_angle, this.step, false)
        }else if (this.direction == RIGHT){
            this.score = FeedbackUtilities.right_waist(this.keypoint!!, this.max_angle, this.step, false)
        }
    }

    private fun updateColorBit(){
        if (this.direction == LEFT){
            this.colorbit = ColorUtilities.left_waist(this.score)
        }else if (this.direction == RIGHT){
            this.colorbit = ColorUtilities.right_waist(this.score)
        }
    }
