...
class AngleSingleLeg: CriteriaBase() {
    ...
    override fun set(maxAngle: Double, step: Double, direction: String) {
        ...
        if (this.max_angle > 160){
            this.good_performance = this.direction + " leg is straight enough"
            this.bad_performance = this.direction + " leg is not straight enough"
        }else if (this.max_angle < 60){
            this.good_performance = this.direction + " leg is curve enough"
            this.bad_performance = this.direction + " leg is not curve enough"
        }else{
            this.good_performance = this.direction + " leg is close to " + this.max_angle.toString() + " degree"
            this.bad_performance = this.direction + " leg is not close to " + this.max_angle.toString() + " degree"
        }
    }

    private fun updateScore(){
        if (this.direction == LEFT){
            this.score = FeedbackUtilities.left_leg(this.keypoint!!, this.max_angle, this.step, false)
        }else if (this.direction == RIGHT){
            this.score = FeedbackUtilities.right_leg(this.keypoint!!, this.max_angle, this.step, false)
        }
    }

    private fun updateColorBit(){
        if (this.direction == LEFT){
            this.colorbit = ColorUtilities.left_leg(this.score)
        }else if (this.direction == RIGHT){
            this.colorbit = ColorUtilities.right_leg(this.score)
        }
    }
