...
class CaturangaDandasana : YogaBase() {

    override val poseName: String = Pose.CaturangaDandasana
    ...
    private fun makeComment(): MutableList<String>{
        ...
        comment!!.add("$arm_score, The Curvature of the Arms " + FeedbackUtilities.comment(arm_score))
    ...
    private fun calculateScore():Double{
        ...
        val right_arm_score = FeedbackUtilities.right_arm(resultArray!!, 90.0, 20.0, true)
        val left_arm_score = FeedbackUtilities.left_arm(resultArray!!, 90.0, 20.0, true)
        arm_score = if(right_arm_score > left_arm_score){right_arm_score}else{left_arm_score}
